...
// 不要なページを無効化
function remove_page_view($query): void
{
  if (!is_admin()) {
...
function eg_settings_api_init(): void
{
  // セクションを追加
...
function eg_setting_callback_function(): void
{
...
// ページ表示前
function read_page_variable(): void
{
  ob_start(static function ($buffer) {
    ...
    // 画像の参照をURL参照に変更
    $buffer = str_replace(['src="src/', 'src="/src/'], 'src="'.get_template_directory_uri().'/src/', $buffer);

    return str_replace(['href="src/', 'href="/src/'], 'href="'.get_template_directory_uri().'/src/', $buffer);
  });
}
